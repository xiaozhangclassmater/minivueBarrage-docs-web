...
export default {
  ...
  methods: {
    ...
    // 添加弹幕
    add(options) {
      this.barrageManager.addBarrage(this.barrageManager.createBarrage(options , true))
    },
    // 重置弹幕
    reset(barrages) {
    ...
    saveClientWidth(clientWidth){
      ...
      this.barrageManager.saveBarrageItemClientWidth(clientWidth , this.currentRow)
  ...
  computed: {
    filterList() {
      return {
        level1: this.barrageManager.getBarrageList().filter(item => item.level === barrageLevel.LEVEL1) || [],
        level2: this.barrageManager.getBarrageList().filter(item => item.level === barrageLevel.LEVEL2) || []
      };
    },
    // 获取弹幕容器高度
